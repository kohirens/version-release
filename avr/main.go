...
func main() {
	...
	case workflowSelector:
		...
		// Validate commit input is a real git ref.
		...
		if !hasSemverTag {
			...
			switch clo.CiCd {
			case circleci.Name:
				if e := circleCiTagWorkflow(branch, client); e != nil {
					mainErr = e
					return
				}
			case github.Name:
				log.Logf("trigger a tag_and_release workflow on GitHub Actions")
				//if e := github.TagWorkflow(branch, client); e != nil {
				//	mainErr = e
				//	return
				//}
			default:
	...
	changLog:
		...
		if len(hasUnreleasedChanges) > 0 {
			...
			pp, e6 := circleci.PipelineParameters(branch, publishChgLogWorkflow)
			if e6 != nil {
				mainErr = e6
				return
			}
			...
			case circleci.Name:
				...
				mainErr = ciClient.TriggerWorkflow(pp)
			case github.Name:
