...
func init() {
	val, k := os.LookupEnv(circleci.EnvApiHost)
	if k {
		ApiHost = val
	}

	val, k = os.LookupEnv(circleci.EnvAppHost)
	if k {
		AppHost = val
	}

	val, k = os.LookupEnv(circleci.EnvToken)
	if k {
		token = val
	}

	val, k = os.LookupEnv(circleci.EnvProjectUsername)
	if k {
		username = val
	}

	val, k = os.LookupEnv(circleci.EnvProjectReponame)
	if k {
		project = val
	}
}
...
func TriggerWorkflow(pp []byte, client *http.Client, eVars map[string]string) error {
